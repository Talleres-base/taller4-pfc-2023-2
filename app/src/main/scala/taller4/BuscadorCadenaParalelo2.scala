...
class BuscadorCadenaParalelo2(cadenaObjetivo: String, alfabeto: Seq[Char]) {
  ...
  */
  ...
  def buscarSecuenciasMSubs(subsecuencias: Seq[Seq[Char]]): Seq[Seq[Char]] = {
    ...
    if (!c1.isEmpty) c1 else Seq(Seq())
  }

  // Función auxiliar que busca secuencias mejoradas a partir de una lista de secuencias
  def busCadMejoradaParAux(secuencias: Seq[Seq[Char]]): Seq[Char] = {
    ...
    if (secuencias.head.isEmpty) Seq()
      ...
      val (c1, c2) = parallel(buscarSecuenciasMSubs(secuencias.take(secuencias.length / 2)), buscarSecuenciasMSubs(secuencias.drop(secuencias.length / 2)))
      ...
      else busCadMejoradaParAux(total)
    }
  }
  ...
  busCadMejoradaParAux(alfabetoEnForma)
  }
  ...
  */
  def turbo(): Seq[Char] = {
    ...
    def buscarSubcadenasRapidoAux(sck: Seq[Seq[Char]], n: Int,combinacion_pasada: Seq[Seq[Char]] = Seq(Seq())): Seq[Seq[Char]]  = {
    ...
    if (n == 2) buscarSubcadenasRapidoAux(alfabetoEnForma, n).head
    else if (n % 2 == 0) {
      val cadenas1 = task{buscarSubcadenasRapidoAux(alfabetoEnForma, n / 2)} 
      val cadenas2 = task{buscarSubcadenasRapidoAux(alfabetoEnForma, (n - 1 ) / 2)} // se fuerza ser  impares
      ...
      val cadenas1 = task {buscarSubcadenasRapidoAux(alfabetoEnForma, n / 2)}
      val cadenas2 = task {buscarSubcadenasRapidoAux(alfabetoEnForma, n - (n / 2))}
  ...
  */
  def turboMejorada(): Seq[Char]  = {
    // Define una función auxiliar recursiva que busca las subcadenas que satisfacen el oráculo
    def turboMejoradaAux(sck: Seq[Seq[Char]], n: Int, secuanciaPasada: Seq[Seq[Char]] = Seq(Seq())): Seq[Seq[Char]]  = {
      if (sck.head.size + 1 == n) {
  ...
  /**
    ...
    */
    ...
    /**
      ...
      */
    def evaluar_arbol(arbol: Trie, n: Int,combinacion_pasada: Seq[Seq[Char]] =Seq(Seq())): Seq[Seq[Char]] = {
